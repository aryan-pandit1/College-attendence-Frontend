...
import "./GPA.css";
import Skeleton from "../Components/Skeleton"; // 👈 Imported Skeleton
...
const GPA = ({ darkMode }) => {
  ...
  const [isSemesterLoading, setIsSemesterLoading] = useState(false); // For switching dropdowns
  ...
  // 2. Wrap initial fetches in loading state
  useEffect(() => {
  ...
  // 3. Wrap semester changes in secondary loading state
  useEffect(() => {
    if (selectedSemester) {
  ...
  const loadGraph = async (targetSemesters = semesters) => {
    ...
    for (const semester of targetSemesters) {
      try {
        ...
        graph.push({ semester: `Sem ${semester.semester_number}`, sgpa: 0 });
      }
    }
    setGraphData(graph);
  };
  ...
  const semesterSubjects = subjects.filter((subject) => {
    ...
    return subject.semester === semester.semester_number;
  });
  ...
  const handleGradeChange = async (courseId, gradePoint) => {
    ...
    try {
      const existingGrade = grades.find(
        (g) => g.course === courseId && g.semester === selectedSemester
      );
  ...
  const calculatedRequiredSGPA = remainingSemesters === 0 ? 0 :
    (targetCGPA * (completedSemesters + remainingSemesters) - currentCGPA * completedSemesters) / remainingSemesters;

  const requiredSGPA = calculatedRequiredSGPA > 10 ? "Impossible" : Math.max(0, calculatedRequiredSGPA).toFixed(2);
    ...
    <div className={`gpa-page ${darkMode ? "forced-dark" : ""}`}>
      ...
      <div className="gpa-top">
        <div className="card trend-card">
          ...
          <div className="chart">
            ...
            {isLoading || graphData.length === 0 ? (
              <div style={{ display: "flex", gap: "16px", height: "150px", alignItems: "flex-end", width: "100%", justifyContent: "center" }}>
                {[1, 2, 3, 4].map(i => <Skeleton key={i} width="40px" height={`${Math.random() * 60 + 40}%`} borderRadius="6px 6px 0 0" />)}
              </div>
            ) : (
              graphData.map((item) => (
        ...
        <div className="card target-card">
          ...
          <div className="target-row">
            ...
            {isLoading ? <Skeleton width="60px" height="32px" borderRadius="6px"/> : (
              <input type="number" value={currentCGPA} onChange={(e) => setCurrentCGPA(Number(e.target.value))} />
            )}
          </div>
          <div className="target-row">
            <span>Completed Semesters</span>
            <input type="number" value={completedSemesters} onChange={(e) => setCompletedSemesters(Number(e.target.value))} />
          </div>
          <div className="target-row">
            <span>Remaining Semesters</span>
            <input type="number" value={remainingSemesters} onChange={(e) => setRemainingSemesters(Number(e.target.value))} />
          </div>
          <div className="target-row">
            <span>Target CGPA</span>
            <input type="number" value={targetCGPA} onChange={(e) => setTargetCGPA(Number(e.target.value))} />
          </div>
          <div className="target-result">
            <p>Required Average SGPA</p>
            {isLoading ? <Skeleton width="80px" height="36px" /> : <h2>{requiredSGPA}</h2>}
          </div>
        </div>
      </div>
      ...
      <div className="gpa-middle">
              ...
              {isLoading || isSemesterLoading ? (
                [1, 2, 3, 4].map((i) => (
                  <tr key={i}>
                    ...
                    <td><Skeleton width="120px" height="32px" borderRadius="6px" /></td>
                    <td><Skeleton width="40px" height="20px" /></td>
                  </tr>
                ))
              ) : semesterSubjects.length === 0 ? (
                <tr>
                  <td colSpan="4">No subjects in this semester.</td>
                </tr>
              ) : (
                semesterSubjects.map((subject) => {
                  ...
                  return (
                    <tr key={subject.id}>
                      ...
                      <td>
                        <select value={currentGradePoint} onChange={(e) => handleGradeChange(subject.id, e.target.value)}>
                          <option value="">Select Grade</option>
                          {gradeOptions.map(([grade, point]) => (
                            <option key={grade} value={point}>{grade}</option>
                          ))}
                        </select>
                      </td>
                      <td>{calculatedPoints}</td>
                    </tr>
                  );
                })
              )}
        ...
        <div className="card grade-card">
          ...
          <table>
            ...
            <tbody>
              {gradeOptions.map(([grade, point]) => (
                <tr key={grade}><td>{grade}</td><td>{point}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      <div className="summary-card">
        <div className="summary-item">
          <span>SGPA</span>
          {isLoading || isSemesterLoading ? <Skeleton width="80px" height="40px" /> : <h2>{estimatedSGPA}</h2>}
        </div>
        <div className="summary-item">
          <span>Total Credits</span>
          {isLoading || isSemesterLoading ? <Skeleton width="60px" height="40px" /> : <h2>{totalCredits}</h2>}
        </div>
        <div className="summary-item">
          <span>CGPA</span>
          {isLoading ? <Skeleton width="80px" height="40px" /> : <h2>{estimatedCGPA}</h2>}
        </div>
      </div>
    </div>
  );
